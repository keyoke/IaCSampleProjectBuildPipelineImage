...
[Environment]::SetEnvironmentVariable("Path", $env:Path + ";$($env:ProgramFiles)\kubectl\" , [EnvironmentVariableTarget]::Machine)
...
[Environment]::SetEnvironmentVariable("Path", $env:Path + ";$($env:ProgramFiles)\aks-engine\", [EnvironmentVariableTarget]::Machine)